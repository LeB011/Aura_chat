...
class TinyFishProvider(ProspectSearchProvider):
    ...
    def _choose_company_name(self, fetched: dict, item: dict, domain: str, criteria: SearchCriteria) -> tuple[Optional[str], int]:
        ...
        # Never manufacture a company name by prefixing the searched industry/domain.
        # If a title begins with the query (e.g. "Administration Favre SA" while the
        # domain brand is Favre), require the rest of the title to align with the domain.
        query=self._normalize(criteria.industry or "")
        for raw_title in (fetched_title, search_title):
            t=self._normalize(raw_title)
            if query and t.startswith(query + " "):
                rest=t[len(query):].strip(" -|:")
                brand=self._normalize(self._domain_brand(domain))
                if rest and brand and brand not in rest and rest not in brand:
                    raw_title=""
