...
class TinyFishProvider(ProspectSearchProvider):
    ...
    Uses TinyFish Search to discover likely first-party business websites, then
    Fetch to enrich each result with public page metadata and generic business
    contact details. Search and Fetch are read-only; Test Mode still blocks all
    real outbound sending elsewhere in Aura.
    """
    key = "tinyfish"
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                r = await client.get(
                    ...
                    params={"query": "electricien Lausanne", "location": "CH", "language": "fr", "page": 0},
                )
            if r.status_code == 200:
                return True, None
    ...
    @staticmethod
    def _clean_company_name(title: Optional[str], domain: str, industry: str) -> str:
        raw = (title or "").strip()
        generic = {"accueil", "home", "contact", "bienvenue", "welcome", "site officiel", "official site"}
        if raw:
            parts = [x.strip() for x in re.split(r"\s+[|–—]\s+|\s+-\s+", raw) if x.strip()]
            parts = [x for x in parts if x.lower() not in generic]
            if parts:
                # Prefer a concise brand/company-looking segment over generic page labels.
                parts.sort(key=lambda x: (len(x) > 90, -len(x)))
                return parts[0][:120]
        if domain:
    ...
    def _extract_phone(text: str) -> Optional[str]:
        # Conservative public-business phone extraction for CH/FR-style numbers.
        patterns = [
    ...
    async def search(self, criteria: SearchCriteria) -> List[dict]:
        ...
        max_results = min(max(1, criteria.max_results), 50)
        ...
        query = f"{criteria.industry} {place} entreprise contact site officiel".strip()
        purpose = (
            f"Trouver les sites officiels d'entreprises de type {criteria.industry} "
            f"dans la zone {place or location} pour une recherche de prospects B2B. "
            "Privilégier les sites propres aux entreprises, pas les annuaires ni les réseaux sociaux."
        )

        headers = {"X-API-Key": key}
        candidates: list[dict] = []
        seen_domains: set[str] = set()
        pages_to_try = min(5, max(1, (max_results + 9) // 10 + 1))
        ...
        async with httpx.AsyncClient(timeout=25.0, follow_redirects=True) as client:
            ...
            # Fetch public pages in batches of 10, TinyFish Fetch's documented max.
            ...
            for start in range(0, len(candidates), 10):
                ...
                fr = await client.post(
                    ...
                    json={
                        ...
                        "purpose": "Lire les informations publiques du site de l'entreprise pour qualifier un prospect B2B.",
                    },
                )
                if fr.status_code == 200:
                    for fetched in (fr.json().get("results") or []):
                        for raw_url in (fetched.get("url"), fetched.get("final_url")):
                            dom = self._domain(raw_url or "")
                            if dom:
                                fetched_by_domain[dom] = fetched
                # Fetch failures are non-fatal: Search result metadata is still real evidence.
        ...
        for item in candidates[:max_results]:
            ...
            final_url = self._ensure_url(fetched.get("final_url") or item.get("url") or "")
            website = self._root_url(final_url)
            company_name = self._clean_company_name(title, domain, criteria.industry)
