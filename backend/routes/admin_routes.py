...
# ---------- V8: per-organization agent entitlements ----------
@router.get("/organizations/{org_id}/agents")
async def organization_agents(org_id: str, user: dict = Depends(get_superadmin)):
    db=get_db()
    if not await db.organizations.find_one({"id":org_id},{"_id":0,"id":1}):
        raise HTTPException(404,"Organisation introuvable")
    return await db.agents.find({"organization_id":org_id},{"_id":0}).sort("name",1).to_list(None)


@router.patch("/organizations/{org_id}/agents/{agent_key}")
async def set_organization_agent(org_id: str, agent_key: str, payload: dict, user: dict = Depends(get_superadmin)):
    db=get_db()
    if not await db.organizations.find_one({"id":org_id},{"_id":0,"id":1}):
        raise HTTPException(404,"Organisation introuvable")
    allowed={k:payload[k] for k in ("enabled","status","minimum_plan") if k in payload}
    if not allowed: raise HTTPException(400,"Aucun changement")
    allowed["updated_at"]=now_iso()
    result=await db.agents.update_one({"organization_id":org_id,"key":agent_key},{"$set":allowed})
    if result.matched_count==0:
        raise HTTPException(404,"Agent introuvable pour cette organisation")
    await _log_platform(db,user["id"],"Accès agent modifié","platform.agent_entitlement",entity_type="organization",entity_id=org_id,target=agent_key,meta=allowed,status="info")
    return await db.agents.find_one({"organization_id":org_id,"key":agent_key},{"_id":0})
