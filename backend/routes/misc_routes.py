...
SECURITY_ALLOWED_KEYS = {
    ...
    "require_verified_identity", "minimum_verification_to_send",
    "block_unverified_contact", "block_personal_email", "audit_every_send",
}


@security_router.patch("")
async def update_security(payload: dict, user: dict = Depends(require_min_role("admin"))):
    db = get_db()
...
async def toggle_kill_switch(payload: dict, user: dict = Depends(require_min_role("admin"))):
    db = get_db()
...
async def list_integrations(user: dict = Depends(get_current_user)):
    ...
    for integ in result:
        if integ.get("key") == "google_places":
            ...
            # Phase 0: standardize on OPENAI_API_KEY (public OpenAI SDK).
            # Keep EMERGENT_LLM_KEY as legacy fallback for orgs still on it.
...
async def validate_integration(integration_key: str, user: dict = Depends(get_current_user)):
    ...
    if integration_key == "openai":
        ok = bool(os.environ.get("OPENAI_API_KEY") or os.environ.get("EMERGENT_LLM_KEY"))
        return {"ok": ok, "status": "connected" if ok else "not_configured",
                "error": None if ok else "OPENAI_API_KEY manquante"}
    if integration_key == "tinyfish":
        ok = bool(os.environ.get("TINYFISH_API_KEY"))
        return {"ok": ok, "status": "connected" if ok else "not_configured",
                "error": None if ok else "TINYFISH_API_KEY manquante"}
...
async def update_integration(integration_key: str, payload: dict, user: dict = Depends(require_min_role("admin"))):
...
async def update_ai(payload: dict, user: dict = Depends(require_min_role("admin"))):
    db = get_db()
...
async def get_usage(user: dict = Depends(get_current_user)):
    """Phase 0B: read-only usage/quotas for the UI."""
    db = get_db()
    org = await db.organizations.find_one({"id": user["organization_id"]}, {"_id": 0})
    return await quotas.usage_summary(org or {"id": user["organization_id"]})


@settings_router.patch("/organization")
async def update_org(payload: dict, user: dict = Depends(require_min_role("owner"))):
    db = get_db()
