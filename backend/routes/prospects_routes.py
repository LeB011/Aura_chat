...
async def create_campaign(criteria: SearchCriteria, user: dict = Depends(get_current_user)):
    ...
    for item in raw:
        ...
        opp = build_opportunity(p.model_dump(), criteria.service_to_sell, criteria.offer or {})
        p.opportunity = opp
...
async def reverify_prospect(prospect_id: str, user: dict = Depends(get_current_user)):
    ...
    opp=build_opportunity(merged,services,offer)
...
async def reverify_batch(payload: dict, user: dict = Depends(get_current_user)):
