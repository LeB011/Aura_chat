...
import { Bot, Sheet, Mail, Send, Map, Database, Workflow, Zap, Webhook, FileSpreadsheet, Code, Search } from "lucide-react";
import { toast } from "sonner";

const ICONS = { Bot, Sheet, Mail, Send, Map, Database, Workflow, Zap, Webhook, FileSpreadsheet, Code, Search };
            ...
            <Card key={i.key} data-testid={`integration-${i.key}`} className="hover:border-foreground/20 transition-colors">
              <CardContent className="p-5">
                ...
                <div className="mt-4">
                  ...
                  {i.key === "tinyfish" && !i.connected && <p className="text-xs text-muted-foreground mt-2">Ajoutez TINYFISH_API_KEY dans les variables d'environnement du backend Render.</p>}
                </div>
              </CardContent>
            </Card>
          );
