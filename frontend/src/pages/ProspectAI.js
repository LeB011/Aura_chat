...
const INDUSTRIES = ["Electricien", "Garage", "Plombier", "Peintre", "Agence immobilière", "Restaurant", "Fiduciaire", "Architecte", "Salon de coiffure", "Cabinet", "PME", "Artisan", "Commerce"];
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input data-testid="form-industry" list="industries" value={form.industry} onChange={(e) => setForm({...form, industry: e.target.value})} required />
          ...
          <Card>
            <CardContent className="p-6 space-y-3">
              ...
              <Select value={form.provider} onValueChange={(v) => setForm({...form, provider: v, sources: [v]})}>
                ...
                <SelectContent>
                  {providers.filter((p) => ["mock", "tinyfish", "google_places"].includes(p.key)).map((p) => (
              ...
              <Card>
                ...
                <AccordionContent>
                  <CardContent className="pt-0 space-y-4">
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      <div className="space-y-1.5"><Label>Domaine d'activité</Label>
                        <Select value={form.filters.business_domain} onValueChange={(v) => setForm({...form, filters: {...form.filters, business_domain: v}})}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>{BUSINESS_DOMAINS.map(([v,l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent>
                        </Select>
                      </div>
