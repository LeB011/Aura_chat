...
import { ArrowLeft, Sparkles, Send, Copy, Mail, Phone, Globe, MapPin, Loader2, CheckCircle, RefreshCw, ShieldCheck, Target, CopyPlus } from "lucide-react";
...
export default function ProspectDetail() {
  ...
  const reverify = async () => {
    setBusy(true);
    try {
      const { data } = await api.post(`/prospects/${id}/reverify`);
      setP(data);
      toast.success("Prospect revérifié et scores recalculés");
    } catch (err) { toast.error(err?.response?.data?.detail || "Revérification impossible"); }
  ...
  const findSimilar = async () => {
    setCloneBusy(true);
    try {
      const { data } = await api.post(`/prospects/${id}/clone-profile`);
      navigate("/prospect-ai", { state: { prefill: data.criteria, mission: data.mission } });
      toast.success("Profil similaire préparé");
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Impossible de préparer une recherche similaire");
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <Badge variant="outline"><ShieldCheck className="w-3 h-3 mr-1" /> Vérif. {p.verification_score || 0}%</Badge>
                    <Button size="sm" variant="outline" onClick={reverify} disabled={busy}>
                      <RefreshCw className={`w-3 h-3 mr-1 ${busy ? "animate-spin" : ""}`} /> Revérifier
                    </Button>
                    <Button size="sm" variant="outline" onClick={findSimilar} disabled={cloneBusy}>
                      {cloneBusy ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <CopyPlus className="w-3 h-3 mr-1" />} Trouver similaires
                    </Button>
                  </div>
          ...
          <Card>
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2"><Target className="w-4 h-4" /><h3 className="font-semibold">Pourquoi ce prospect ?</h3></div>
                <Badge className="text-sm">Priorité {p.aura_grade || "—"} · {p.commercial_score || 0}/100</Badge>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
                {[['Fit',p.fit_score],['Besoin',p.need_score],['Données',p.data_quality_score],['Contact',p.contactability_score]].map(([l,v]) => (
                  <div key={l} className="rounded-md border p-2"><div className="text-xs text-muted-foreground">{l}</div><div className="font-semibold">{v || 0}%</div></div>
                ))}
              </div>
              <ul className="text-sm space-y-1.5 list-disc pl-5">
                {(p.why_this_prospect || p.opportunity?.why_this_prospect || []).map((x,i)=><li key={i}>{x}</li>)}
              </ul>
              {(p.verified_observations || []).length > 0 && <div className="text-xs text-muted-foreground">
                <span className="font-medium text-foreground">Observations vérifiées :</span> {(p.verified_observations || []).join(' · ')}
              </div>}
              {(p.opportunity?.recommended_angle || p.opportunity?.next_best_action) && (
                <div className="grid sm:grid-cols-2 gap-2 pt-1">
                  {p.opportunity?.recommended_angle && <div className="rounded-md border p-3 text-sm"><div className="text-xs font-medium text-muted-foreground mb-1">Angle recommandé</div>{p.opportunity.recommended_angle}</div>}
                  {p.opportunity?.next_best_action && <div className="rounded-md border p-3 text-sm"><div className="text-xs font-medium text-muted-foreground mb-1">Prochaine action</div>{p.opportunity.next_best_action}</div>}
                </div>
              )}
            </CardContent>
          </Card>
