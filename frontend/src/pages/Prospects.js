...
export default function Prospects() {
  ...
  const [auraGrade, setAuraGrade] = useState("all");
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) => {
      const matchesText = !q || (i.company_name || "").toLowerCase().includes(ql) || (i.city || "").toLowerCase().includes(ql);
      const matchesGrade = auraGrade === "all" || i.aura_grade === auraGrade;
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          <div className="flex flex-col md:flex-row gap-2">
            ...
            <Select value={auraGrade} onValueChange={setAuraGrade}>
              <SelectTrigger className="md:w-36"><SelectValue placeholder="Priorité Aura" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Toutes priorités</SelectItem>
                <SelectItem value="A+">A+ · Très haute</SelectItem>
                <SelectItem value="A">A · Haute</SelectItem>
                <SelectItem value="B">B · Bonne</SelectItem>
                <SelectItem value="C">C · À analyser</SelectItem>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-muted-foreground text-xs uppercase tracking-wider border-b border-border">
                  ...
                  <th className="px-4 py-2">Aura</th>
                    ...
                    <tr key={p.id} data-testid={`prospect-row-${p.id}`} className="border-b border-border hover:bg-accent/40">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <Badge className="text-[10px]">{p.aura_grade || "—"}</Badge>
                          <span className="mono text-[10px] text-muted-foreground">{p.commercial_score || 0}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-2 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-muted-foreground">Priorité Aura : {p.aura_grade || "—"} · {p.commercial_score || 0}/100</div>
