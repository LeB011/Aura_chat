...
const TOGGLES = [
  ...
  ["require_verified_identity", "Identité vérifiée obligatoire", "Bloque la préparation/contact si le prospect n'atteint pas le seuil de vérification."],
  ["block_unverified_contact", "Bloquer les contacts non vérifiés", "Évite les contacts lorsque les données de l'entreprise sont insuffisantes."],
  ["block_personal_email", "Bloquer les emails personnels", "Privilégie les adresses professionnelles pour la prospection B2B."],
  ["audit_every_send", "Journaliser chaque tentative d'envoi", "Conserve une piste d'audit des actions sensibles."],
];
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div><Label>Vérification minimale avant contact (%)</Label>
              <Input type="number" min="0" max="100" value={s.minimum_verification_to_send ?? 50} onChange={(e) => setS({...s, minimum_verification_to_send: parseInt(e.target.value || "0")})} onBlur={() => update({ minimum_verification_to_send: s.minimum_verification_to_send })} />
            </div>
          </div>
        </CardContent>
      </Card>
